...
using AzureMcp.Commands.Foundry;
...
public class FoundryService : BaseAzureService, IFoundryService
{
            ...
            while (pageCount < maxPages)
            {
                ...
                try
                {
                    ...
                    foreach (var summary in response.Summaries)
                    {
                        try
                        {
                            summary.DeploymentInformation.FreePlayground = summary.PlaygroundLimits != null;
                            if (!string.IsNullOrEmpty(summary.Publisher) &&
                                summary.Publisher.Equals("openai", StringComparison.OrdinalIgnoreCase))
                            {
                                summary.DeploymentInformation.OpenAI = true;
                            }
                            else
                            {
                                if (summary.AzureOffers != null)
                                {
                                    summary.DeploymentInformation.ServerlessEndpoint =
                                        summary.AzureOffers.Contains("standard-paygo");

                                    summary.DeploymentInformation.ManagedCompute =
    ...
    public async Task<CognitiveServicesAccountDeploymentResource> DeployModel(string deploymentName, string modelName, string modelFormat,
