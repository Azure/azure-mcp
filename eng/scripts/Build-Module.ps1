...
param(
    ...
    [switch] $CleanBuild,
    [switch] $BuildNative,
    [ArgumentCompleter({(Get-ChildItem "$PSScriptRoot/../../cli" -Directory).Name})]
    [ValidateScript({$_ -in (Get-ChildItem "$PSScriptRoot/../../cli" -Directory).Name})]
    [string] $ServerName,
...
$projectFile = Get-ChildItem "$RepoRoot/cli/$serverName/src/" -Recurse -Filter '*.Cli.csproj'
...
try {
    ...
    foreach ($os in $operatingSystems) {
        foreach ($arch in $architectures) {
            ...
            $package = Get-Content "$RepoRoot/cli/$serverName/package.json" -Raw
