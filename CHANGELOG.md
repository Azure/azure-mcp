...
- Added support for batch setting access tier for multiple Azure Storage blobs via the `azmcp-storage-batch-settier` command. This command efficiently changes the storage tier (Hot, Cool, Archive) for multiple blobs simultaneously in a single operation. [[#735](https://github.com/Azure/azure-mcp/issues/735)]
