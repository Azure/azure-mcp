...
namespace AzureMcp.Storage.LiveTests
{
    ...
    : CommandTestsBase(liveTestFixture, output), IClassFixture<LiveTestFixture>
    {
        ...
        public async Task Should_set_blob_tier_batch()
        {
            ...
            var result = await CallToolAsync(
                ...
                new()
                {
                    ...
                    { "container-name", "bar" },
                    { "tier-name", "Cool" },
                    { "blob-names", "testblob1.txt testblob2.txt" }
                });
        ...
        [InlineData("--account-name test --container-name test")] // Missing required parameters
        public async Task Should_Return400_WithInvalidBatchSetTierInput(string args)
        {
            var result = await CallToolAsync(
                $"azmcp_storage_batch_settier {args}");
