...
| azmcp-loadtesting-test-create | Create a basic URL test using the following endpoint URL <test-url> that runs for 30 minutes with 45 virtual users. The test name is <sample-name> and the load testing resource is <load-test-resource> in the resource group <rgName> in my subscription |
