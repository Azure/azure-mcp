...
public class McpRuntimeTests
{
    ...
    public async Task CallToolHandler_DelegatesToToolLoader()
    {
        ...
        mockTelemetry.Received(1).StartActivity(TelemetryConstants.ActivityName.ToolExecuted, Arg.Any<Implementation?>());
        Assert.Equal(ActivityStatusCode.Ok, activity.Status);
    ...
    public async Task ListToolsHandler_WhenToolLoaderThrows_PropagatesException()
    {
        ...
        mockTelemetry.Received(1).StartActivity(TelemetryConstants.ActivityName.ListToolsHandler, Arg.Any<Implementation?>());
